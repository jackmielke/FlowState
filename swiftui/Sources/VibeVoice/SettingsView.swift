...
struct SettingsView: View {
    ...
    /// Dictation: the explainer, not another wall of switches.
    ///
    /// This tab exists because dictation is the one feature whose behaviour you cannot
    /// infer by looking at it — the words leave this app and land somewhere else, and when
    /// that fails it fails silently in someone else's text field. So the tab's job is to
    /// answer three questions in order: what does holding the key do, is the one
    /// permission that matters actually granted, and what will it do to my words.
    ///
    /// The examples below are generated by calling `Dictation.tidy` for real rather than
    /// being written out as strings. Hard-coded examples drift from the implementation the
    /// first time someone edits the replacement table, and then the tab is lying.
    @ViewBuilder
    private var dictationTab: some View {

            section("Hold to talk") {
                caption("Hold the dictation key, say your sentence, let go. The words are typed into whatever already had your cursor \u{2014} a Slack message, a commit body, a search field \u{2014} without bringing \(kAssistantDisplayName) to the front.")
                caption("Nothing is sent while the key is up, and releasing it always ends the utterance. A second press is ignored until the first one has landed, so two half-sentences can never interleave.")
            }
            ...
            section("What it does to your words") {
                caption("Spoken punctuation becomes punctuation, filler at the start of a sentence is dropped, and sentences get their capital letters back.")
                ForEach(Self.dictationExamples, id: \.self) { spoken in
                    VStack(alignment: .leading, spacing: 2) {
                        Text("\u{201C}\(spoken)\u{201D}")
                            .font(.system(size: 11.5))
                            .foregroundStyle(Theme.text.opacity(0.55))
                        Text(Dictation.tidy(spoken))
                            .font(.system(size: 12.5))
                            .foregroundStyle(Theme.text)
                    }
                    .frame(maxWidth: .infinity, alignment: .leading)
            ...
            section("Where it stops") {
                caption("Punctuation words are ambiguous and always will be \u{2014} \u{201C}the period of time\u{201D} contains the word \u{201C}period\u{201D}. An article in front of the word is treated as a signal that you meant the noun. That catches the common cases and misses the exotic ones, which is the honest trade rather than a silent one.")
            }
    }

    /// Sample utterances for the tab above. Deliberately chosen to show one of each
    /// behaviour, including the one that is intentionally left alone.
    private static let dictationExamples = [
        "hello there comma how are you question mark",
        "um so I was thinking new line let's ship it",
    ...
    private var accessTab: some View {
            ...
            section("Anywhere") {
                ...
                SegmentedPicker(options: HotkeyCombo.wakeChoices.map { (value: $0.id, label: $0.label) }
                                        + [(value: "", label: "Off")],
                                selection: Binding(
                ...
                HStack {
                    ...
                    NeatToggle(isOn: Binding(
                        ...
                        set: { state.settings.startAtLogin = $0; state.applyStartAtLogin() }))
                        .accessibilityLabel("Start at login")
                }
                caption(LoginItem.isAvailable
                        ? "A keyboard shortcut belongs to a running program — macOS has no way to launch an app because a key was pressed. So this is what makes the wake key work \"even when \(kAssistantDisplayName) is closed\": it is not closed, it is sitting in the menu bar with nothing open. Closing the window no longer quits it, for the same reason."
                        : "Unavailable — this is the bare binary from .build rather than the built app bundle. Run ./build.sh and launch FlowState.app.")
                if LoginItem.needsApproval {
                    caption("macOS is holding this until you allow it. Open System Settings › General › Login Items and switch \(kAssistantDisplayName) on.")
                }
                caption("If you would rather it stayed quit: bind your key in Shortcuts, Raycast or Alfred to `open flowstate://connect` instead. Those are always running, so they can launch \(kAssistantDisplayName) cold and land in the same place this key does.")
