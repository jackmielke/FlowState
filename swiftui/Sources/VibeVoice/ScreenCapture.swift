...
enum ScreenPermission: Equatable {
    ...
    var bannerText: String {
        switch self {
        case .granted:      return "Screen Recording is on for FlowState."
        case .denied:       return "Screen Recording is off for FlowState — turn it on in System Settings."
        case .needsRestart: return "Screen Recording is granted, but FlowState must be relaunched to pick it up."
...
enum ScreenCapture {
    ...
    /// "FlowState" row to toggle at all.
    ...
    /// Every display FlowState is allowed to look at, in a stable order.
    ///
